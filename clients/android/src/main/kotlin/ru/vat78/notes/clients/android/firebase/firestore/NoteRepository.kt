...
//                .limit(100)
    ...
    override suspend fun updateNote(note: Note) {
        Log.i("NoteRepository", "Update note ${note.id}, timestamp: ${note.lastUpdate}")
    ...
    private fun deleteLink(batch: WriteBatch, childNoteId: String, parentNoteId: String, timestamp: Long = Instant.now().epochSecond) {
        batch.delete(
            firestore.collection(USER_COLLECTION)
                .document(user.id)
                .collection(NOTES_COLLECTION)
        ...
        updateLink(batch, childNoteId, parentNoteId, deleted = true, timestamp)
    }

    private fun insertLink(batch: WriteBatch, childNoteId: String, parentNoteId: String, timestamp: Long = Instant.now().epochSecond) {
        batch.set(
            firestore.collection(USER_COLLECTION)
                .document(user.id)
                .collection(NOTES_COLLECTION)
        ...
        updateLink(batch, childNoteId, parentNoteId, deleted = false, timestamp)
    }
    ...
    override suspend fun deleteLinks(links: List<NoteLink>) {
        withContext(Dispatchers.IO) {
            firestore.runBatch { batch ->
                links.forEach {
                    deleteLink(batch, it.childId, it.parentId, timestamp = 0)
                }
            }
        }
    }
    ...
    override suspend fun saveSyncingLinks(links: List<NoteLink>) {
        withContext(Dispatchers.IO) {
            firestore.runBatch { batch ->
                links.forEach {
                    insertLink(batch, it.childId, it.parentId, timestamp = 0)
                }
            }
        }
    }
